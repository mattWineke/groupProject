...
from classes.Button import startButton
...
def main():
    global running
    running = True
    #variable for intro screen
    introControl=0
    #sound and music set up
    s='sound'
    music=pygame.mixer.music.load(os.path.join(s,'Space Walk- Pluto’s Theme.mp3'))
    pygame.mixer.music.play(-1)                              
    ouch=pygame.mixer.Sound(os.path.join(s,'playerDamage.aiff'))
    # Main loop
    while running:
        # Event loop
        ...
        #intro control and its variable
        introControl+=1
        introScreen(introControl)
...
def introScreen(i):
    if i == 3:
        ...
        while press is False:
            myFont=pygame.font.SysFont("Comic Sans",32)
            title=myFont.render("Pluto's Pursuit!",True,'white') #title text
            surface.blit(title,(80,100))
            button = startButton(90, 550, 200, 50, 'white', "Start!", BLACK)  #OBJECT INSTANCE 
            for event in pygame.event.get():
